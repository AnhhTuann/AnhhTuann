...
```html
<!DOCTYPE html>
<!-- Khai báo HTML5, bắt buộc ở dòng đầu -->
<html>
  <head>
...
- `var`: Chỉ bị nhốt bởi function. Nếu nằm trong `if {}` hoặc `for {}`, nó vẫn thoát ra ngoài được.(Function Scope)
...
- Dùng `const` cho giá trị cố định (hằng số).

🚨 **BẪY PHỎNG VẤN VỚI `const`**:
Nhiều người nghĩ `const` là hoàn toàn không thể thay đổi. Đúng là bạn không thể "gán lại" (không thể dùng dấu `=`), **NHƯNG** nếu giá trị đó là một Object hoặc Array (giống như một cái túi), bạn **VẪN ĐƯỢC PHÉP thêm/bớt/sửa đồ vật bên trong** cái túi đó. Cốt lõi là bạn không được đổi sang cái túi khác.

```javascript
// 1. VỚI OBJECT
const user = { name: "A" };
user.name = "B"; // ✅ ĐƯỢC: Bạn thò tay vào túi, đổi chữ A thành B.
user.age = 20;   // ✅ ĐƯỢC: Bạn bỏ thêm tuổi vào túi.
user = {};       // ❌ LỖI: Bạn vứt cái túi cũ đi và thay bằng cái túi mới.

// 2. VỚI ARRAY (MẢNG)
const list = [1, 2, 3];
list.push(4);    // ✅ ĐƯỢC: Nhét thêm số 4 vào mảng cũ. (Thành [1, 2, 3, 4])
list[0] = 99;    // ✅ ĐƯỢC: Sửa số 1 thành số 99.
list = [5, 6];   // ❌ LỖI: Vứt mảng cũ đi, gán bằng mảng mới.
```
> **Bản chất**: `const` chỉ khóa chặt **địa chỉ bộ nhớ** của cái túi, chứ không khóa chặt **nội dung** bên trong túi.
...
@layer utilities {
  .text-red {
    color: red;
  } /* Luôn thắng dù specificity thấp */
}
```
